...
export class FrosthavenNewMonsterSelect extends ActorSheet {
  ...
  /** @override */
  get template() {
    return `systems/frosthaven/templates/actor/actor-${this.actor.type}-sheet.html`;
  }
  ...
  _prepareActorData(context) {
    ...
    for (let [k, v] of Object.entries(context.data.saves)) {
      v.label = game.i18n.localize(CONFIG.FROSTHAVEN.saves[k]) ?? k;
    }
  }
  ...
  _prepareCharacterData(context) {
    ...
    for (let [k, v] of Object.entries(context.data.abilities)) {
      v.label = game.i18n.localize(CONFIG.FROSTHAVEN.abilities[k]) ?? k;
    }
    // Handle money.
    for (let [k, v] of Object.entries(context.data.money)) {
      v.label = game.i18n.localize(CONFIG.FROSTHAVEN.money[k]) ?? k;
    }
  }
